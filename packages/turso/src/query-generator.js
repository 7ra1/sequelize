...
import { TursoQueryGeneratorTypeScript } from './query-generator-typescript.internal.js';

export class TursoQueryGenerator extends TursoQueryGeneratorTypeScript {
  createTableQuery(tableName, attributes, options) {
    // SQLite/Turso supports unique keys via CREATE UNIQUE INDEX, but this is handled separately after table creation
    // For now, we accept the option but don't fail if it's provided
    if (options) {
      rejectInvalidOptions(
        'createTableQuery',
        this.dialect,
        CREATE_TABLE_QUERY_SUPPORTABLE_OPTIONS,
        new Set(['uniqueKeys']),  // Support uniqueKeys option
        options,
      );
    }
