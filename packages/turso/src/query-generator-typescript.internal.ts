...
import { TursoQueryGeneratorInternal } from './query-generator.internal.js';
 ...
 */
export class TursoQueryGeneratorTypeScript extends AbstractQueryGenerator {
  readonly #internals: TursoQueryGeneratorInternal;

  constructor(
    dialect: TursoDialect,
    internals: TursoQueryGeneratorInternal = new TursoQueryGeneratorInternal(dialect),
  ) {
  ...
  showIndexesQuery(_tableName: TableOrModel) {
    // Turso does not support PRAGMA index_list, so return a query that gets index info from sqlite_master
    // This is a workaround since Turso has limited PRAGMA support
    // For now, we return an empty result set by using a query that won't match anything
    return `SELECT NULL as 'seq', NULL as 'name', NULL as 'unique', NULL as 'origin', NULL as 'partial' LIMIT 0`;
  }
